...
# Preserve even unreferenced source vertices; Blender OBJ importer may drop them.
parts=[]
for line in (folder/'vds67-reference.obj').read_text().splitlines():
    fields=line.split()
    if not fields: continue
    if fields[0]=='o': parts.append({'name':fields[1], 'v':[], 'f':[]})
    elif fields[0]=='v': parts[-1]['v'].append([float(x) for x in fields[1:4]])
    elif fields[0]=='f': parts[-1]['f'].append([int(x)-1 for x in fields[1:]])
offset=0
for part in parts:
    mesh=bpy.data.meshes.new(part['name'])
    mesh.from_pydata(part['v'], [], [[i-offset for i in f] for f in part['f']])
    mesh.update()
...
for record in meta['objects']:
    ...
    obj.color = {'shell': (0.25, 0.5, 0.7, 1), 'deck': (0.7, 0.7, 0.7, 1),
                 'roof': (0.8, 0.5, 0.2, 1)}.get(record['role'], (0.65, 0.69, 0.72, 1))
...
report = {'status': 'PASS_IMPORT_SAVE_REOPEN', 'blender_version': bpy.app.version_string,
          ...
          'scope': str(len(meta['objects'])) + ' objects, cached vertex/face counts, source UUID, metre bounding boxes before and after reopen',
